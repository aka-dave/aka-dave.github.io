...
var url = `http://localhost:3000/api/Author/ImagesAll?person=${artist}`
...
function update () {
 ...
 async function justName(enterCard) {
    ...
    const urlIMG = `http://localhost:3000/api/noAuthor/images?title=${enterCard}`;  // enterCard will be the query parameter here
    ...
    const urlCard_images = `http://localhost:3000/api/author/card_images?title=${enterCard}`;  // handling the draw card 
...
async function justAuthor(author){
    ...
    for(let i of resAll){              
        if(i.name == (author)){
            ...
            const urlCard_images = `http://localhost:3000/api/Author/ImagesAll?person=${author}`
            const promiseCardIMG = await fetch(urlCard_images);
             resCardIMG = await promiseCardIMG.json();
...
function handleImages (resIMG) {   // simplified to work for any number of rows. Has 1 bug but I'm not sure how to replecate it at the moment. works most of the time
...
for(let i=galleryLen - difference; i<galleryImgs.length; i++) {
    ...
    galleryImgs[i].src = `images/DebugDog.jpg`;
    ...
    for(let i=0; i<Math.abs(difference); i++) { 
        ...
        if(typeof resIMG[galleryLen + i + 1] == "undefined" && typeof resIMG[galleryLen + i] != "undefined") {  // for a 1 problem you can ommit the second condition
            ...
            var r = i % 3;
            var end = 3 - (1+r);  // how many columns you need to loop through
